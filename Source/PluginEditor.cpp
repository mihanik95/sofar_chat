...
const std::vector<SOFARAudioProcessorEditor::PresetData>
    SOFARAudioProcessorEditor::presetData = {
        {"Open Air", 120.0f, 120.0f, 60.0f, 0.20f, 20.0f}, // Vast outdoor space
        {"Large Hall", 45.0f, 28.0f, 10.0f, 0.35f,
         18.0f}, // Concert hall acoustics
        {"Cathedral", 70.0f, 35.0f, 25.0f, 0.30f,
         18.0f}, // Huge stone cathedral
        {"Medium Studio", 12.0f, 10.0f, 4.5f, 0.45f,
         21.0f}, // Controlled recording studio
        {"Drum Room", 8.0f, 7.0f, 3.0f, 0.55f,
         22.0f}, // Tight but lively drum booth
        {"Underground Cave", 60.0f, 30.0f, 20.0f, 0.10f,
         15.0f}, // Dark reflective cave
        {"Vehicle Interior", 4.0f, 2.5f, 2.0f, 0.80f,
         25.0f},                                   // Confined vehicle cabin
...
SOFARAudioProcessorEditor::SOFARAudioProcessorEditor(SOFARAudioProcessor &p)
  ...
  // Set larger window size for clean grid layout (4x2 grid for 8 controls +
  // presets)
  setSize(960, 560);
  ...
  // Set up main title
  titleLabel.setText("SOFAR - Spatial Distance Effect",
  ...
  // Version label
  versionLabel.setText("v0.0088", juce::dontSendNotification);
  ...
  signalCategoryLabel.setColour(juce::Label::textColourId,
                                juce::Colour(0xff4a90e2));
  addAndMakeVisible(signalCategoryLabel);

  roomCategoryLabel.setText("ROOM CONTROL (affected by presets)",
                            juce::dontSendNotification);
  ...
  roomCategoryLabel.setColour(juce::Label::textColourId,
  ...
  volumeCompensationAttachment.reset(
      new juce::AudioProcessorValueTreeState::SliderAttachment(
          audioProcessor.parameters, "volumeCompensation",
          volumeCompensationSlider));
  ...
  distanceSlider.setColour(juce::Slider::thumbColourId,
                           juce::Colour(0xff4a90e2));
  distanceSlider.setColour(juce::Slider::rotarySliderFillColourId,
                           juce::Colour(0xff4a90e2));

  // Room dimensions - different shades of blue/purple
  roomLengthSlider.setColour(juce::Slider::thumbColourId,
                             juce::Colour(0xff7b68ee));
  roomLengthSlider.setColour(juce::Slider::rotarySliderFillColourId,
                             juce::Colour(0xff7b68ee));

  roomWidthSlider.setColour(juce::Slider::thumbColourId,
                            juce::Colour(0xff9370db));
  roomWidthSlider.setColour(juce::Slider::rotarySliderFillColourId,
                            juce::Colour(0xff9370db));

  roomHeightSlider.setColour(juce::Slider::thumbColourId,
                             juce::Colour(0xff8a2be2));
  roomHeightSlider.setColour(juce::Slider::rotarySliderFillColourId,
                             juce::Colour(0xff8a2be2));

  airAbsorptionSlider.setColour(juce::Slider::thumbColourId,
                                juce::Colour(0xff50c878));
  airAbsorptionSlider.setColour(juce::Slider::rotarySliderFillColourId,
                                juce::Colour(0xff50c878));

  volumeCompensationSlider.setColour(juce::Slider::thumbColourId,
                                     juce::Colour(0xff32cd32));
  volumeCompensationSlider.setColour(juce::Slider::rotarySliderFillColourId,
                                     juce::Colour(0xff32cd32));

  temperatureSlider.setColour(juce::Slider::thumbColourId,
                              juce::Colour(0xffff7f50));
  temperatureSlider.setColour(juce::Slider::rotarySliderFillColourId,
                              juce::Colour(0xffff7f50));

  panningSlider.setColour(juce::Slider::thumbColourId,
                          juce::Colour(0xffffa500));
  panningSlider.setColour(juce::Slider::rotarySliderFillColourId,
                          juce::Colour(0xffffa500));

  // Pointer calculation in default LookAndFeel subtracts 90°, so use 0 → 2π to
  // make front (pointer) at 12-o'clock
  panningSlider.setRotaryParameters(0.0f, juce::MathConstants<float>::twoPi,
                                    false);
  ...
  prevPresetButton.setColour(juce::TextButton::buttonColourId,
                             juce::Colour(0xff2a2a2a));
  prevPresetButton.setColour(juce::TextButton::buttonOnColourId,
                             juce::Colour(0xff4a90e2));
  addAndMakeVisible(prevPresetButton);

  nextPresetButton.setButtonText(">");
  nextPresetButton.onClick = [this] { navigatePreset(1); };
  nextPresetButton.setColour(juce::TextButton::buttonColourId,
                             juce::Colour(0xff2a2a2a));
  nextPresetButton.setColour(juce::TextButton::buttonOnColourId,
                             juce::Colour(0xff4a90e2));
  ...
  volumeCompensationSlider.setTooltip(
      "Compensates for perceived loudness loss with distance (%).");
  ...
  airAbsorptionSlider.setTooltip(
      "High-frequency attenuation due to air humidity (%).");
  temperatureSlider.setTooltip(
      "Air temperature (°C) – affects speed of sound.");

  // New height slider
  setupSlider(heightSlider, "Height", "%");
  heightAttachment =
      std::make_unique<juce::AudioProcessorValueTreeState::SliderAttachment>(
          audioProcessor.parameters, "height", heightSlider);
  heightSlider.setColour(juce::Slider::thumbColourId, juce::Colour(0xff20b2aa));
  heightSlider.setColour(juce::Slider::rotarySliderFillColourId,
                         juce::Colour(0xff20b2aa));
  heightSlider.setTooltip(
      "Vertical position of source (0% floor – 100% ceiling).");
}
...
void SOFARAudioProcessorEditor::updateBackgroundImage() {
  backgroundImage =
      juce::Image(juce::Image::RGB, getWidth(), getHeight(), false);
...
void SOFARAudioProcessorEditor::paint(juce::Graphics &g) {
  ...
  juce::Rectangle<int> signalBounds(40, 150, getWidth() - 80,
                                    (getHeight() - 180) / 2 - 15);
  g.fillRoundedRectangle(signalBounds.toFloat(), 8.0f);

  // Room control section background
  juce::Rectangle<int> roomBounds(40, sectionDividerY + 15, getWidth() - 80,
                                  (getHeight() - 180) / 2 - 15);
...
void SOFARAudioProcessorEditor::resized() {
  ...
  // Signal Controls Section (3 controls: Distance, Panning, Volume
  // Compensation)
  {
    int gridWidth = signalControlsArea.getWidth() / 4;
    int controlSize =
        std::min(gridWidth - 20, signalControlsArea.getHeight() - 20);
    controlSize = std::min(controlSize, 140);

    auto positionSignalControl = [&](juce::Component &control, int col) {
      int x = signalControlsArea.getX() + col * gridWidth +
              (gridWidth - controlSize) / 2;
      int y = signalControlsArea.getY() +
              (signalControlsArea.getHeight() - controlSize) / 2;
  ...
  // Room Controls Section (5 controls: Room Length, Width, Height, Air
  // Absorption, Temperature)
  {
    int gridWidth = roomControlsArea.getWidth() / 5;
    int controlSize =
        std::min(gridWidth - 15, roomControlsArea.getHeight() - 20);
    ...
    auto positionRoomControl = [&](juce::Component &control, int col) {
      int x = roomControlsArea.getX() + col * gridWidth +
              (gridWidth - controlSize) / 2;
      int y = roomControlsArea.getY() +
              (roomControlsArea.getHeight() - controlSize) / 2;
      control.setBounds(x, y, controlSize, controlSize);
    };
...
void SOFARAudioProcessorEditor::setupSlider(juce::Slider &slider,
  ...
  // Fix parameter formatting - don't add suffix here, it's handled in parameter
  // creation slider.setTextValueSuffix(suffix); // Remove this line
...
void SOFARAudioProcessorEditor::setupPresets() {
  ...
  presetsComboBox.setColour(juce::ComboBox::backgroundColourId,
                            juce::Colour(0xff2a2a2a));
  presetsComboBox.setColour(juce::ComboBox::textColourId, juce::Colours::white);
  presetsComboBox.setColour(juce::ComboBox::outlineColourId,
                            juce::Colour(0xff4a90e2));
  presetsComboBox.setColour(juce::ComboBox::arrowColourId,
                            juce::Colour(0xff4a90e2));
...
void SOFARAudioProcessorEditor::loadPreset(int presetIndex) {
  if (presetIndex >= 0 && presetIndex < presetData.size()) {
    ...
    audioProcessor.parameters.getParameterAsValue("roomLength")
        .setValue(preset.roomLength);
    audioProcessor.parameters.getParameterAsValue("roomWidth")
        .setValue(preset.roomWidth);
    audioProcessor.parameters.getParameterAsValue("roomHeight")
        .setValue(preset.roomHeight);
    audioProcessor.parameters.getParameterAsValue("airAbsorption")
        .setValue(preset.airAbsorption);
    audioProcessor.parameters.getParameterAsValue("temperature")
        .setValue(preset.temperature);

    // SIGNAL CONTROL parameters are deliberately NOT changed to preserve user's
    // signal positioning:
...
void SOFARAudioProcessorEditor::navigatePreset(int direction) {
  int currentIndex =
      presetsComboBox.getSelectedItemIndex(); // 0 = "Select Preset..."
